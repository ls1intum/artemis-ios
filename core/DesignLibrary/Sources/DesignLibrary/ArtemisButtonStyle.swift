...
public struct ArtemisButton: ButtonStyle {
    ...
    public func makeBody(configuration: ButtonStyleConfiguration) -> some View {
        configuration.label
            ...
            .background(isEnabled ? buttonBackgroundColorWrapper : Color.Artemis.buttonDisabledColor)
            .foregroundColor(buttonTextColorWrapper)
            .clipShape(Capsule())
            .overlay(Capsule().stroke(buttonTextColorWrapper).opacity(buttonPriority == .secondary ? 1 : 0))
