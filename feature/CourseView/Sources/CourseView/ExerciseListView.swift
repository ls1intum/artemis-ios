...
struct ExerciseListView: View {
    ...
    init(course: Course) {
        self._viewModel = StateObject(wrappedValue: ExerciseListViewModel(course: course))
    }

    var body: some View {
        List {
            ForEach(viewModel.weeklyExercises) { weeklyExercise in
                Section(weeklyExercise.id.description) {
                    ForEach(weeklyExercise.exercises) { exercise in
                        Text(exercise.baseExercise.title ?? "TODO") // TODO: why optional
                    }
                }
            }
        }
